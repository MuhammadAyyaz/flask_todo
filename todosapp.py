...
@app.route('/todo/v1_v01/tasks/my/<string:id>', methods=['GET'])
def findone(id):
    frame=mongo.db.todo
...
@app.route('/todo/v1_v01/tasks/my', methods=['GET'])
def findall():
    frame=mongo.db.todo
    out=[]
...
@app.route('/todo/v1_v01/tasks/my', methods=['POST'])
def adding():
    frame=mongo.db.todo
    ID=request.json['ID']
...
@app.route('/todo/v1_v01/tasks/my/<string:id>', methods=['DELETE'])
def deletion(id):
    frame=mongo.db.todo
    dt=frame.find_one({'ID':id})
    frame.remove(dt)
...
@app.route('/todo/v1_v01/tasks/my/<string:id>',methods=['PUT'])
def updation(id):
    frame=mongo.db.todo
    dt=frame.find_one({'ID':id})
